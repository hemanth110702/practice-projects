        ...
        async (position) => {
          ...
          try {
            const nav_response = await fetch(
              `https://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&limit=5&appid=${app_id}`
            );
...
export const getWeatherData = async (name, country) => {
  try {
    const w_url = `https://api.openweathermap.org/data/2.5/weather?q=${name},${country}&appid=${app_id}`;
    const f_url = `https://api.openweathermap.org/data/2.5/forecast?q=${name},${country}&appid=${app_id}`;
